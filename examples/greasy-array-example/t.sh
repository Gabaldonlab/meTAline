...
singularity run --cleanenv /gpfs/projects/bsc40/project/pipelines/meTAline/meTAline-1.1.0/metaline.sif metaline-prepare-greasy-array-job \
    --metaline-rule "all" \
    --basedir ./test_output \
    --metaline-cmd "module load singularity && singularity run --cleanenv /gpfs/projects/bsc40/project/pipelines/meTAline/meTAline-1.1.0/metaline.sif metaline" \
    --greasy-cmd "/apps/GPP/GREASY/2.2.4.1/INTEL/IMPI/bin/greasy" \
    ...
    --joblist-size 2 `# Max. number of jobs per each node in the Greasy array.` \
