...
if reference_genome != None or reference_genome != "null":
    rule bowtie2:
        ...
        shell:
            ...
            "echo 'bowtie2-align-s --local --threads {threads} -x {params.ref} -1 {input.read1} -2 {input.read2} -D {params.D} -R {params.R} -N {params.N} -L {params.L} -i {params.i} --score-min {params.score_min} | samtools sort -@ {threads} -O BAM -o {output.out} 2> {log};'"
