...
####
    ...
    def register_input(self, parser):
        ...
        input_group.add_argument('--protein_db', dest="protein_db" , metavar="protein_db", default=self.protein_db, help='Humann database to do the translation search (by deafult this is by-passed).Default \"%s\".' % self.protein_db)
        input_group.add_argument('--n_db', dest="n_db" , metavar="n_db", default=self.n_db, help='Humann database to do the nucleotide search (based on already built annotations.Default \"%s\".' % self.n_db)
    ...
    def register_output(self, parser):
        ...
        output_group.add_argument('--metaphlan4-out', dest="metaphlan4_out", help='Out directory of the Metaphlan4 and Humann analysis.Default "/%s"' % self.metaphlan4_out)
    ...
    def register_fastqc(self, parser):
        ...
        fastqc_group.add_argument('--adapters', dest="adapters", metavar="adapters", default=self.adapters, help='Adapter sequences to be checked in the quality assessment. Default %s.' % self.adapters)
    ...
    def register_kraken2(self, parser):
        ...
        kraken2_group.add_argument('--kraken2-cores', type=int, dest="kraken2_cores", metavar="kraken2_cores", default=self.kraken2_cores, help='Number of threads to run the Kraken2 taxonomic assignment step. Default %s.' % self.kraken2_cores)
####
