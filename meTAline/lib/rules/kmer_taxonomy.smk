...
rule Kraken2:
    ...
    benchmark:
        os.path.join(benchmark_dir, (sample +".kraken2.benchmark.txt"))
    threads:
        config["Kraken2"]["kraken2_cores"]
...
rule Krona:
    ...
    benchmark:
        os.path.join(benchmark_dir, (sample +".krona.benchmark.txt"))
    shell:
...
rule extract_reads:
    ...
    benchmark:
        os.path.join(benchmark_dir, (sample +".extract_reads.benchmark.txt"))
    threads: 4
    shell:
